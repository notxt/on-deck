...
customElements.define("dragon-combo", El);

export const createDragonCombo = (): BattleView => {
  const el = new El();

  const update: BattleView["update"] = (battle) => {
    el.cards = battle.dragon.deck.play.map((card) => new CardEl(card));
  };
